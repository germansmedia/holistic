...
fn draw_rgb<T: Pixel>(image: &mut Image<T>,px: usize,py: usize,r: i32,g: i32,b: i32) {
	*image.pixel_mut(usizev2::new(px,py)) = T::new_rgb(clamp(r,0,255) as u8,clamp(g,0,255) as u8,clamp(b,0,255) as u8);
}
...
/// Test if a slice is a JPEG image.
/// # Details
/// `jpeg::test` checks if the given slice is a JPEG image, and return the dimensions of the image.
/// # Arguments
/// * `src`: Slice to check.
/// # Returns
/// * `Some(size)`: The slice is a JPEG image, dimensions are given in `size`.
/// * `None`: The slice is not a JPEG image.
/// # Example
/// ```
/// if let Some(size) = jpeg::test(&buffer) {
///     println!("This is a JPEG image of {}x{} pixels.",size.x,size.y);
/// }
/// ```
...
/// Decode a JPEG image.
/// # Details
/// `jpeg::decode<T>` decodes a JPEG-encoded slice into pixels of type `T`.
/// # Arguments
/// * `src`: Encoded slice.
/// # Returns
...
/// # Example
/// ```
/// if let Ok(image) = jpeg::decode::<RGBA8>(&buffer) {
...
/// Encode a JPEG image.
/// # Details
/// `jpeg::encode<T>` encodes pixels of type `T` into a JPEG image.
/// # Arguments
...
/// if let Ok(buffer) = jpeg::encode(&image) {
///     // do something with the image
/// }
/// ```
/// > TODO, development comments:
